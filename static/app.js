...
${meta.hosted ? `<div class="card"><b>Your CVs are kept on this server</b> under <b>${esc(meta.person)}</b>. Use <b>Open a CV file…</b> to bring in a file (for example one ChatGPT wrote) and <b>Download backup</b> to keep a copy.<div class="actions" style="margin-bottom:0"><button class="secondary" onclick="downloadBackup()">Download backup</button></div></div>` : `<div class="card"><b>Your CV files are here:</b><br><code>${esc(meta.folder)}</code><div class="actions" style="margin-bottom:0"><button class="secondary" onclick="openFolder()">Open this folder</button></div><p class="hint" style="margin:8px 0 0">Any <code>.json</code> file you put in this folder shows up in the <b>Open CV</b> list within a few seconds. The file name becomes the CV name.</p></div>`}
...
async function leave() { await fetch('/api/leave', {method: 'POST'}); location.href = '/'; }
function downloadBackup() { downloadBlob(new Blob([JSON.stringify(cv, null, 2)], {type: 'application/json'}), ((profiles.find(p => p.id === profile) || {}).label || 'CV').replace(/[^a-z0-9]+/gi, '-') + '.json'); }
