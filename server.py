...
SOURCE = LOCAL_SOURCE if LOCAL_SOURCE.exists() else SAMPLE_SOURCE
...
def save_cv(cv: dict) -> None:
    ...
    with tempfile.NamedTemporaryFile(dir=LOCAL_SOURCE.parent, prefix=".cv-", delete=False) as temp:
        temp.write(encoded)
        temp_path = Path(temp.name)
    try:
        os.replace(temp_path, LOCAL_SOURCE)
    finally:
        if temp_path.exists():
            temp_path.unlink()
...
async function save(){let r=await fetch('/api/cv',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(cv)}),d=await r.json();status.textContent=r.ok?'Saved locally.':d.errors.join(' ')}
...
Promise.all([fetch('/api/cv').then(r=>r.json()),fetch('/api/templates').then(r=>r.json())]).then(([x,t])=>{cv=x;templates=t;render()});</script></body></html>'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route == "/": return self.send(APP_HTML.encode(), "text/html; charset=utf-8")
        if route == "/api/cv": return self.send(json.dumps(load_cv()).encode(), "application/json")
    ...
    def do_POST(self):
        try: cv = self.read_json(); document = render_html(cv)
        except (json.JSONDecodeError, ValueError) as exc: return self.send(str(exc).encode(), "text/plain", HTTPStatus.BAD_REQUEST)
        route = urlparse(self.path).path
    ...
    def do_PUT(self):
        if urlparse(self.path).path != "/api/cv": return self.send(b"Not found", "text/plain", HTTPStatus.NOT_FOUND)
...
def main():
    ...
    print("CV editor: http://127.0.0.1:8765 (local only)")
